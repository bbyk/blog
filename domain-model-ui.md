...
Although most modern languages have OOP stuff in it, developers often tend to use objects in a functional way. It is common thing nowadays that you can see a lot of classes working essentially as utilities. They take data in arguments of its methods and change them. This is what is often called [anemic domain model](http://martinfowler.com/bliki/AnemicDomainModel.html).
...
What you see in UI here is essentially a UI component that has 2 modes. It can either render the file name or edit it. Then, being in edit mode it keeps track of text entered, selection, copy/paste etc.

All the stuff is going to be controled by your View Model that essentially represents your domain logic for this piece of UI. Let's take a look at the state your View model will maintain. For simplicity sake I will skip selection though:
...
Suppose user presses a key and that is the event that will be eventually handled in a controller. And the controller has to consult model. If in read mode and 'e' is presssed it will ask model to switch to edit mode. By contrast, if in edit mode, controller will ask model to append the character to the *temporaryFileName* if allowed character is provided or length is within the range:
...
*ViewModel* can be considered as a subset of Domain Model that encloses UI specific whereas *FileDomain* addresses just persistence concerns, rules and core behavior.

From *FileDomain* perspective the class is agnostic in which way it will be eventually used. So I would not recommend to unify logic of Domain Model and View Model into the same class. Instead you can use aggregation as I showed in the examples.